...
The current adapter is `models/kitten.js`, using the pinned [Kitten WebGPU engine](https://github.com/svenflow/kitten-tts-webgpu/tree/35f31049363ea39464dc05d42c1135b5c9e3235f). It owns model loading, GPU event handling, the dictionary/rules frontend, number expansion, voice selection, and splitting expanded text to the model's context limit. The model and Bella voice match the earlier Micro release. Model and runtime pins update only through a tested release; upstream changes are not adopted automatically. Pocket CPU and other alternatives remain explicit developer comparisons.
