...
This bundle contains the unmodified Pocket forward pass from [jax-js](https://github.com/ekzhang/jax-js/blob/${revision}/website/src/routes/tts/pocket-tts.ts), revision \`${revision}\`, by Eric Zhang (MIT). This is a community browser port, not Kyutai's official Python runtime. The CPU comparison adapter uses only the WASM backend with FP32 computation; the experimental comparison adapter explicitly selects WebGPU FP16 or FP32. Both follow the demo's prompt preparation and sampling defaults, collect PCM instead of playing it, bound generation, and release decoder states between passages. Neither adapter automatically changes backends. WASM kernels are compiled by this bundle, so no additional runtime binaries are downloaded. Without cross-origin isolation, WASM runs on one worker thread; shared-memory parallelism is optional. The compact FP16 weight download requires JavaScript Float16Array support even when computation uses FP32. The CPU comparison adapter caches pinned model assets in browser CacheStorage when available; storage failures do not prevent generation.
